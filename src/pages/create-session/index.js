...
const redirectNext = async () => {
  ...
  return (
    ...
    overflow-y-scroll
         bg-[#e7e7e7]
    " onClick={() => localStorage.setItem('status', 'pending')}>
    <div className='flex items-center sm:shadow-md '>
      <h1 className='text-center  text-[#0e1129] heading font-kanit text-[1.5em] sm:text-3xl font-bold mr-2'>Registeration form</h1>
       <Image src={logo} alt="EDUSN" width={120} height={60}/></div>
      <form className="glass-container backdrop-blur-lg 
   
       md:w-[80%] w-[95%] min-h-[98%]
      items-center  bg-opacity-0.1
       px-8 py-2  gap-y-6 sm:gap-y-5 grid grid-cols-1 sm:grid-cols-2 gap-x-10" onSubmit={handleSubmit(onSubmit)}>
   ...
   <ModalComponent isOpen={modalOpen} onClose={closeModal}>
          <h2 className="text-lg font-semibold text-black  text-center">Terms and Conditions (Agreement)</h2>
          {policiesDatas.map((item, index) => (
            <li className='sm:leading-8 text-gray-750 text-black leading-5 mt-2 text-sm' key={index}>{item}</li>
...
<Checker required={true}/>
          ...
          <button disabled={!isChecked} className="btn btn-active btn-accent" onClick={redirectNext}>Confirm</button></div>
               ...
               <div className="flex flex-col" >
                  <h1 className='text-black'>Old Grade Report Card or Birth Certificate (မွေးစာရင်း)</h1>
            <input
              disabled={isLoading}
                    required
                    accept="image/*"
        ...
        <div  className=" w-full flex items-center justify-end sm:col-start-2 h-full ">
          <Button
            disabled={isLoading}
            color="success"
            className="
            ...
            sm:h-full w-1/3 sm:w-1/5 sm:p-2 mb-5 sm:mb-0 text-white bg-deep-purple-700 hover:bg-deep-purple-800 
